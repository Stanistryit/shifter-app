...
// SHIFTS: Create (з перевіркою дублікатів)
...
router.post('/tasks/delete', async (req, res) => { 
    ...
    // FIX: Передаємо req.session.userId, а не рядок
    const perm = await handlePermission(req, req.session.userId); 

    if(perm.status === 'pending') { 
        const reqDoc = await Request.create({
            type: 'del_task', // Новий тип запиту
...
// REQUESTS ACTION (UPDATED: Added del_task support)
router.post('/requests/action', async (req, res) => { 
    ...
    if(action === 'approve'){ 
        ...
        if(r.type === 'del_task') await Task.findByIdAndDelete(r.data.id); // Нова обробка

        notifyUser(r.createdBy, `✅ Схвалено`); 
    } else {
        notifyUser(r.createdBy, `❌ Відхилено`); 
    }
