...
router.post('/tasks', async (req, res) => { const perm = await handlePermission(req, req.session.userId); if(perm.status === 'pending') { const reqDoc = await Request.create({type:'add_task', data:req.body, createdBy:perm.user.name}); sendRequestToSM(reqDoc); return res.json({success:true, pending:true}); } await Task.create(req.body); const { date, name, title, isFullDay, start, end } = req.body; let dur = "Весь день"; if (!isFullDay && start && end) { const [h1, m1] = start.split(':').map(Number); const [h2, m2] = end.split(':').map(Number); dur = `${((h2 + m2/60) - (h1 + m1/60)).toFixed(1)} год.`; } notifyUser(name, `📌 <b>Задача:</b> ${title}\n📅 ${date}\n⏳ ${dur}`); logAction(perm.user.name, 'add_task', title); res.json({ success: true }); });
