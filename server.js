...
const axios = require('axios'); // Не забудь встановити це локально!
...
// ============================================================
// --- AIR RAID ALERTS SYSTEM (Сумський район) ---
// ============================================================
...
async function getWorkersOnShiftNow() {
    // Отримуємо точний час у Києві
    const kyivTimeStr = new Date().toLocaleString("en-US", {timeZone: "Europe/Kiev", hour12: false});
    const now = new Date(new Date().toLocaleString("en-US", {timeZone: "Europe/Kiev"}));
    
    // Формат YYYY-MM-DD для пошуку в базі (трохи складно через часові пояси, тому форматуємо вручну)
    const year = now.getFullYear();
    const month = String(now.getMonth() + 1).padStart(2, '0');
    const day = String(now.getDate()).padStart(2, '0');
    const todayStr = `${year}-${month}-${day}`;
    
    // Парсимо поточні години і хвилини
...
async function checkAirAlert() {
    if (!bot) return;
    try {
        // Запит до публічного API мапи тривог
        const response = await axios.get('https://alarmmap.online/assets/json/_alerts.json'); 
        const alerts = response.data; // Це масив об'єктів активних тривог
        
        // Фільтрація: Шукаємо "Сумська область" АБО "Сумський район" АБО "Сумська ... громада"
        const isSumyAlert = alerts.some(a => {
            const name = (a.n || "").toLowerCase(); // .n - це назва регіону
            return name.includes("сумська область") || 
                   name.includes("сумський район") ||
                   (name.includes("сумська") && name.includes("громада"));
        });
        ...
        if (isSumyAlert && !isAlertActive) {
            ...
            for (const name of workers) notifyUser(name, `🚨 <b>ПОВІТРЯНА ТРИВОГА! (Сумський р-н)</b>\n\nТерміново в укриття!`);
        } 
            ...
            for (const name of workers) notifyUser(name, `✅ <b>ВІДБІЙ ТРИВОГИ</b>\n\nМожна повертатись до роботи.`);
        }
...
app.post('/api/requests/action', async (req, res) => { 
    const {id,action}=req.body; const r=await Request.findById(id); if(!r)return res.json({success:false}); 
    if(action==='approve'){ 
        if(r.type==='add_shift'){await Shift.create(r.data); notifyUser(r.data.name, `📅 <b>Зміна підтверджена!</b>\n${r.data.date}`);}
        if(r.type==='del_shift')await Shift.findByIdAndDelete(r.data.id);
        if(r.type==='add_task'){await Task.create(r.data); notifyUser(r.data.name, `📌 <b>Задача підтверджена!</b>\n${r.data.title}`);}
        if(r.type==='del_task')await Task.findByIdAndDelete(r.data.id);
        if(r.type==='add_event'){await Event.create(r.data); notifyAll(`📢 <b>Подія підтверджена!</b>\n${r.data.title}`);}
    } 
    const sIcon=action==='approve'?'✅':'❌'; const sTxt=action==='approve'?'Схвалено':'Відхилено';
    notifyUser(r.createdBy, `${sIcon} <b>Твій запит було ${sTxt}</b>\n\nТип: ${r.type}`);
