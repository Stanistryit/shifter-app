...
// ============================================================
// --- AIR RAID ALERTS SYSTEM (Safe & Robust) ---
// ============================================================

let isAlertActive = false;

// Хто зараз на зміні (Київський час)
...
async function checkAirAlert() {
    if (!bot) return;
    try {
        // ВИПРАВЛЕННЯ: Додаємо Headers, щоб прикинутися браузером (Chrome)
        // Це запобігає блокуванню запиту сервером
        ...
        // БЕЗПЕКА: Перевіряємо, чи це точно масив
        if (!Array.isArray(alerts)) {
            console.error("Alert API returned non-array data:", typeof alerts);
            return;
        }
        
        // Фільтрація: Сумська область АБО Сумський район АБО Сумська громада
        // .n - це поле з назвою регіону в цьому API
        ...
        if (isSumyAlert && !isAlertActive) {
            ...
            for (const name of workers) {
                notifyUser(name, `🚨 <b>ПОВІТРЯНА ТРИВОГА! (Сумський р-н)</b>\n\nТерміново в укриття!`);
            }
        } 
